...
def initialize_model(model_type, in_channel, num_gesture, device, 
    ...
    """
    load_path = f"MetaLearn_{model_type}_Session_{session}_Subject_{subject}_Input_{input_type}_Train_type_{training_type}.pth"
...
def run_kd(path, session, subject, num_repetitions, input_type, training_type, 
   ...
   # Data preparation
    ...
    save_dir = os.path.join(
        save_path,
        f"KD_{model_type}_Session_{session}_Subject_{subject}_Input_{input_type}_Train_type_{training_type}.pth"
    )
